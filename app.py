...
import joblib  # Library baru untuk load model .pkl
...
# ==========================================
# 1. FUNGSI: LOAD MODEL (FORMAT .PKL)
# ==========================================
@st.cache_resource
def load_model():
    try:
        # Load model dari file .pkl (Jauh lebih stabil dari JSON)
        # Pastikan file model_banjir.pkl sudah ada di folder yang sama
        model = joblib.load("model_banjir.pkl")
        return model
    except Exception as e:
        st.error(f"Gagal memuat model 'model_banjir.pkl'. Pastikan file sudah diupload! Error: {e}")
        return None
...
def get_gemini_advice(risk_percent, rr, wind, humidity, status_level):
    ...
    try:
        # Cari model secara dinamis agar tidak error 404
        available_models = list(genai.list_models())
        
        # Prioritas pencarian: Flash -> Pro -> Apa saja
        ...
        - Curah Hujan Akumulasi (3 Hari): {rr} mm
        ...
        1. **Analisis Situasi**: Jelaskan kenapa angka hujan/angin tersebut berbahaya/aman.
        ...
        Gunakan bahasa Indonesia yang mendesak, empatik, dan mudah dipahami.
        """
        ...
        return f"⚠️ Gagal menghubungi Gemini ({active_model}): {str(e)}"
...
# TAMPILAN UTAMA (UI)
# ==========================================
st.title("🌊 Pusat Komando Banjir AI")
st.markdown("### Powered by XGBoost + Google Gemini AI")

model = load_model()

if model is None:
    st.error("⚠️ Model AI tidak ditemukan. Silakan upload file 'model_banjir.pkl' ke folder ini.")
else:
